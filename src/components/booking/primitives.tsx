...
export function friendlyTimezone(tz?: string): string {
  if (!tz || tz === "America/Sao_Paulo" || tz === "America/Fortaleza" || tz === "America/Recife" || tz === "America/Bahia") {
    return "Horários no fuso de Brasília";
  }
  if (tz === "America/Manaus" || tz === "America/Boa_Vista") return "Horários no fuso do Amazonas (GMT-4)";
  if (tz === "America/Cuiaba" || tz === "America/Campo_Grande") return "Horários no fuso do MS/MT (GMT-4)";
  if (tz === "America/Rio_Branco") return "Horários no fuso do Acre (GMT-5)";
  if (tz === "America/Noronha") return "Horários no fuso de F. de Noronha (GMT-2)";
...
export function PublicFrame({
  ...
  children: ReactNode;
  color?: string;
  company?: { name: string; category?: string | null; logoUrl?: string | null; slug?: string; address?: string | null };
}) {
  return (
    <div
    ...
    >
      <header className={styles.header}>
        <div className={styles.headerInner}>
            ...
            {company && (
              <>
                ...
                <div className={styles.headerCompany}>
                  {company.logoUrl ? (
                    <img src={company.logoUrl} alt="" className={styles.headerCompanyLogo} />
                  ) : (
                  ...
                  <div className={styles.headerCompanyText}>
                    <span className={styles.headerCompanyName}>{company.name}</span>
                    {company.category && (
                      <span className={styles.headerCompanyCategory}>{company.category}</span>
                    )}
                  </div>
                </div>
              </>
          ...
          <a href="/meus-agendamentos" className={styles.headerLink} title="Acessar meus agendamentos">
            <UserRound size={16} />
      ...
      <footer className={styles.footer}>
        <div className={styles.footerInner}>
          <span>Agendamento online seguro com <strong>Nova(e)</strong></span>
