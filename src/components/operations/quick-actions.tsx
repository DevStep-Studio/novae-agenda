        ...
        <section className="operations-waitlist" aria-labelledby="waitlist-title">
          ...
          {waiting.map(w => (
            <div key={w.id} className="waitlist-row">
              ...
              {w.clientPhone && <a className="btn btn-secondary" target="_blank" rel="noreferrer" href={`https://wa.me/${w.clientPhone.replace(/\D/g, "").length <= 11 ? "55" : ""}${w.clientPhone.replace(/\D/g, "")}`}>WhatsApp</a>}
            </div>
          ))}
        </section>
      )}
