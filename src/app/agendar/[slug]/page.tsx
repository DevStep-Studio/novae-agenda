...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {company}=await load((await params).slug);return {title:`Agendar com ${company.name} | Novae`,description:company.description||`Reserve seu horário online com ${company.name}.`,openGraph:{title:`Agendar com ${company.name} | Novae`,description:company.description||`Reserve seu horário online com ${company.name}.`,...(company.logoUrl?.startsWith("https://")?{images:[company.logoUrl]}:{})}};}
