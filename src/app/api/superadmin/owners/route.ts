...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const reason = body.reason || "Exclusão em massa via Super Admin";

    if (!ids.length) {
      return Response.json({ error: "Nenhum estabelecimento selecionado para exclusão." }, { status: 400 });
    }

    const result = await AdminService.bulkDeleteOwners(
      ids,
      mode,
      reason,
      { id: gate.auth.user.userId, email: gate.auth.user.email },
    ...
    return Response.json({
      ...result,
      message: `${result.deletedCount} estabelecimentos ${mode === "hard" ? "excluídos definitivamente" : "desativados"} com sucesso.`,
    });
  } catch (error: any) {
    console.error("[Superadmin API Bulk Delete Owners] Error:", error);
    return Response.json({ error: error.message || "Erro ao excluir estabelecimentos em massa." }, { status: 500 });
  }
}
